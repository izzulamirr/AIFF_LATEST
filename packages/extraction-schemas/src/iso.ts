          ...
          properties: {
            ...
            size: {
              ...
              description:
                ...
                "CONFIRMED REAL MISREAD -- THE OPPOSITE CASE IS JUST AS REAL A RED FLAG: two welds sitting right next to each other at a \"200x80NS\" weldolet both got recorded as size 200, because their tight physical proximity made it easy to read both as \"on the main run\" without checking which side of the fitting each one is actually welded to -- the second one was really welded onto the weldolet's own OUTLET (the 80mm branch side), not the main run. A reducing branch point almost always has its nearby welds split across BOTH sizes, one on each side of the fitting -- when two or more welds are described as sitting at or near the SAME reducing weldolet/tee and they all come out the SAME size, that is a sign to re-examine which physical side each weld mark is really welded to before finalizing, not a coincidence to record and move past.",
            },
            location_note: {
              type: "string",
              description:
